...
# ✅ HARD-CODED CONFIG
# =========================================================
BOT_TOKEN = "8689265142:AAE-2NCf2oWNOPmt7_zoq2HiEIjtTMx6mQ8"  # set in Pella Env Vars
...
def _patch_m_db_init():
    ...
    # Per-admin: score reply toggle (default ON)
    if not _table_has_column(conn, "users", "score_reply_on"):
        cur.execute("ALTER TABLE users ADD COLUMN score_reply_on INTEGER NOT NULL DEFAULT 1")

    # Per-admin: topic anchor (chat_id + message_id of the topic header message)
    if not _table_has_column(conn, "users", "topic_anchor_chat"):
        cur.execute("ALTER TABLE users ADD COLUMN topic_anchor_chat INTEGER")
    if not _table_has_column(conn, "users", "topic_anchor_msg"):
        cur.execute("ALTER TABLE users ADD COLUMN topic_anchor_msg INTEGER")
...
def _parse_topic_cmd(full_text: str):
    ...
    Returns (topic_text, target_type, serial, do_pin)
      target_type: 'c' (channel) or 'g' (group) or None
      serial: int or None
      do_pin: bool
    """
    # Strip command prefix
    text = re.sub(r'^[./]topic\b\s*', '', (full_text or '').strip(), flags=re.IGNORECASE)

    # Check if last word is 'pin'
    do_pin = False
    words = text.split()
    if words and words[-1].lower() == 'pin':
        do_pin = True
        words = words[:-1]
        text = ' '.join(words)

    # Check if last remaining word is c<N> or g<N>
    target_type = None
    serial = None
    if words:
        last = words[-1]
        m = re.match(r'^([cCgG])(\d+)$', last)
        if m:
            target_type = m.group(1).lower()
            serial = int(m.group(2))
            words = words[:-1]

    # Reconstruct topic text preserving original newlines
    # We know we've consumed: optional "pin" and "c<N>"/"g<N>" from the end.
    # Rebuild from original stripped text.
    stripped = re.sub(r'^[./]topic\b\s*', '', (full_text or '').strip(), flags=re.IGNORECASE)
    # Remove trailing "pin" and/or "c<N>"/"g<N>" tokens from the raw stripped text
    tail_pattern = rf'(\s+[cCgG]{serial})?' + (r'\s+pin' if do_pin else '') + r'\s*$'
    if serial is not None:
        tail_pattern = rf'\s+[cCgG]{serial}(\s+pin)?\s*$'
        topic_text = re.sub(tail_pattern, '', stripped, flags=re.IGNORECASE).strip()
    else:
        if do_pin:
            topic_text = re.sub(r'\s+pin\s*$', '', stripped, flags=re.IGNORECASE).strip()
        else:
            topic_text = stripped.strip()

    return topic_text, target_type, serial, do_pin
...
async def _send_score_msg(
    ...
    score_text = _score_reply_text(ok_count)
    anchor_chat, anchor_msg = _get_topic_anchor(admin_id)
    reply_to = anchor_msg if (anchor_chat == chat_id and anchor_msg) else first_post_msg_id
    try:
...
async def _cmd_topic_m(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    .topic <text> c<serial> [pin]  — send topic header to channel
    .topic <text> g<serial> [pin]  — send topic header to group
    Reply to a photo+caption then: .topic c<serial> [pin]
    """
    admin_id = update.effective_user.id
    msg = update.message
    if not msg:
    ...
    topic_text, target_type, serial, do_pin = _parse_topic_cmd(full_text)
    ...
    if not target_type or serial is None:
        await safe_reply(update, (
            ...
            "After sending, all quizzes will reply to this topic message.\n"
    ...
    try:
        if reply_msg:
            # Send the replied-to content as the topic header
            if reply_msg.photo:
                caption = topic_text or reply_msg.caption or ""
                ...
                text_to_send = topic_text or reply_msg.text or ""
                if not text_to_send:
                    await warn(update, "Empty Content", "No text found to send as topic.")
                    return
                sent = await context.bot.send_message(chat_id=target_chat_id, text=text_to_send)
        else:
            if not topic_text:
                await warn(update, "No Content",
                    "Provide topic text after the command, or reply to a message.")
                return
            sent = await context.bot.send_message(chat_id=target_chat_id, text=topic_text)
        ...
        await ok_html(update, "✅ Topic Sent",
            ...
            f"All upcoming quiz posts will reply to this topic message.\n"
            f"Use <code>.cleartopic</code> to remove this anchor.")
